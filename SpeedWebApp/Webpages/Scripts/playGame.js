...
async function postJSON(url = '', data = {}) {
    const response = await fetch(url, {
        method: 'POST',
        mode: 'cors', // cors for cross origin
        cache: 'no-cache',
        ...
        body: JSON.stringify(data) // data MUST be a JSON or will be angry
    }).then((data) => {
        return data.text();
    }).then((data) => {
        // console.log(data);
...
function shuffleDeck() {
    ...
    // shuffle the deck and assign pile locations to be.
    // the cards with the lower "order" value sit closer to the top of all closed piles.
    for (let numCards = defaultDeck.length - 1; numCards >= 0; numCards--) {
        let randCardIdx = Math.floor(Math.random() * (numCards + 1));
        let card = defaultDeck.splice(randCardIdx, 1)[0];
        initDeckOrder.push({
            card: card,
            order: numCards,
            location: "",
    ...
    for (let i = 0; i < 20; i++) {
        initDeckOrder[i]["location"] = "hostDeck";
    }
    for (let i = 20; i < 40; i++) {
        initDeckOrder[i]["location"] = "guestDeck";
    }
    for (let i = 40; i < 46; i++) {
        initDeckOrder[i]["location"] = "leftTiebreakerPile";
    }
    for (let i = 46; i < 52; i++) {
        initDeckOrder[i]["location"] = "rightTiebreakerPile";
    }
    guestCards = initDeckOrder.filter(
        (card) => card["location"] == "guestDeck"
    );
    guestCards.sort((card1, card2) =>
        card1["order"] < card2["order"] ? -1 : 1
    );
    for (let i = 0; i < 5; i++) {
        guestCardIdx = initDeckOrder.findIndex(
            (card) => card == guestCards[i]
        );
        initDeckOrder[guestCardIdx]["location"] = "guestCard" + (i + 1);
    }

    hostCards = initDeckOrder.filter(
        (card) => card["location"] == "hostDeck"
    );
    hostCards.sort((card1, card2) =>
        card1["order"] < card2["order"] ? -1 : 1
    );
    for (let i = 0; i < 5; i++) {
        hostCardIdx = initDeckOrder.findIndex(
            (card) => card == hostCards[i]
        );
        initDeckOrder[hostCardIdx]["location"] = "hostCard" + (i + 1);
    }
    return initDeckOrder;
}
let initDeckOrder = shuffleDeck();
...
function logKeypress(pressedKey) {
    matchEvents["keystrokes"].push({
        ...
        timestamp: (Date.now() - matchEvents["startTime"])
    });
}
$(document).ready(() => {
    freezeAll = true;
    ...
    matchEvents["startTime"] = Date.now(); // stores date as milliseconds since epoch
